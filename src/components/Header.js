...
const Header = () => {
  ...
  //   window.location.href =  `${process.env.NEXT_PUBLIC_URL}/linkedin-login`;
  // };
  ...
  return (
    ...
    {/* {isLoading ? (
     ...
     <div className="headerCont max-w-[1160px] bg-white rounded-full mx-auto">
       <div className="mainCont flex items-center justify-between gap-5 py-3 md2:py-0">
        ...
        <div className="hidden md2:flex items-center justify-center gap-5 h-full top-auto right-auto text-main-dark bg-transparent translate-y-1">
          ...
          <Link href="/uniboard" className="flex items-center justify-center font-[600] w-fit h-[65px] -translate-y-1 px-2 transition hover:bg-main-dark hover:text-white">Dashboard</Link>
        </div>
        {/* {isLoggedIn ? (
        <button onClick={handleDashboardClick} className="hidden md2:flex button2 px-5 py-2.5 rounded-full bg-main transition hover:bg-main-dark text-white w-fit -mr-2">Uniboard</button> 
        // <Link href="/uniprep"  className="flex button2 px-5 py-2.5 rounded-full bg-main transition hover:bg-main-dark text-white w-fit -mr-2">Dashboard</Link>
        )
         : (<Link href="#" onClick={handleLinkedInAuth} className="hidden md2:flex button2 px-5 py-2.5 rounded-full bg-main transition hover:bg-main-dark text-white w-fit -mr-2">Sign in with LinkedIn</Link> ) }
     ...
     <div className={`${menu} transition flex md2:hidden gap-7 flex-col fixed top-0 right-0 text-main-dark bg-white h-screen w-[250px] overflow-y-auto z-[70] px-5 py-3`}>
          ...
          <Link href="/uniboard" className="font-[600] w-fit pb-2">Dashboard</Link>
         </div>
